...
export default function TutorialPage() {
  ...
  return (
    <div style={styles.container}>
      ...
      <ul style={styles.scrollContainer}>
        {data.map((k, i) => (
          <ul key={i} style={styles.item}>
            <strong>{k.sym}</strong>: {k.desc}. Example: <code style={styles.code}>{k.ex}</code>
          </ul>
        ))}
      </ul>
      ...
      <label htmlFor="notes" style={styles.label}>Previous Notes</label>
      <ul style={styles.noteList}>
        {notes.map((n, i) => (
          <li key={i} style={styles.noteItem}>
            <span style={styles.noteText}>{n}</span>
            <button onClick={() => handleDeleteNote(i)} style={styles.deleteBtn}>×</button>
          </li>
        ))}
      </ul>
    </div>
